...
/**
 * The {@link FlinkSchemaHistory} only stores the latest schema of the monitored tables. When
 * recovering from the checkpoint, it should apply all the tables to the {@link DatabaseSchema},
 * which doesn't need to replay the history anymore.
 *
 * <p>Considering the data structure maintained in the {@link FlinkSchemaHistory} is much different
 * from the {@link FlinkDatabaseHistory}, it's not compatible with the {@link FlinkDatabaseHistory}.
 * Because it only maintains the latest schema of the table rather than all history DDLs, it's
 * useful to prevent OOM when meet massive history DDLs.
 */
public class FlinkSchemaHistory implements SchemaHistory {
