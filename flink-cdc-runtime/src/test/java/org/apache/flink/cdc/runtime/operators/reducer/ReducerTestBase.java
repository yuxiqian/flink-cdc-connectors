...
/** Some common schema reducing test utilities & functions. */
public abstract class ReducerTestBase {
    ...
    protected static <OP extends AbstractStreamOperator<E>, E extends Event, T extends Throwable>
            LinkedList<StreamRecord<E>> runInHarness(
                    ...
                    Function<OP, EventOperatorTestHarness<OP, E>> harnessCreator,
                    BiConsumerWithException<OP, EventOperatorTestHarness<OP, E>, T> closure)
                    throws T, Exception {
        OP operator = opCreator.get();
        try (EventOperatorTestHarness<OP, E> harness = harnessCreator.apply(operator)) {
            harness.open();
    ...
    protected static StreamRecord<PartitioningEvent> wrap(Event payload, int from, int to) {
        return new StreamRecord<>(new PartitioningEvent(payload, from, to));
    }
}
